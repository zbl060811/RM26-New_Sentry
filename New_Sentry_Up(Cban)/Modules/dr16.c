...
{
    // HAL_UART_Receive_DMA(&huart3,DJI_DR16_Buffer,18);
    HAL_UARTEx_ReceiveToIdle_DMA(&huart3, dr16->buffer, sizeof(dr16->buffer)); // 串口空闲中断接收DMA
...
//    DJI_DR16_Data.RC.CH2 = (((int16_t)buffer[2] >> 6) | ((int16_t)buffer[3] << 2) | ((int16_t)buffer[4] << 10)) & 0x07FF;
...
//    DJI_DR16_Data.Mouse.Z = ((int16_t)buffer[8]) | ((int16_t)buffer[9] << 8);
