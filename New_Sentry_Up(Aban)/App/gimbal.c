...
void Gimbal_Init(void)
{
	...
	Pid_Set(&Gimbal.yaw_motor.pid_speed, 150, 25, 0, 0, 20000);
...
void Gimbal_Pitch_Calc(void)
{
    ...
    #define SCAN_SPEED          50.0f    // 扫描速度（每周期增量）
    ...
    if(Robot.status.control_mode == FIRE_MODE_AUTO)
    {
        ...
        else  // target_found == 0
        {
            ...
            // 检查是否到达扫描边界
            // 【修正】检查是否到达扫描边界（+200度 或 -200度）
            if(mg_motor.params.angle >= 13000)  // 200度 = 200 * 100 = 20000（因为单位是0.01度）
            {
                mg_motor.params.angle = 13000;   // 限制在最高点
                scan_direction = -1;              // 改为向下
            }
            else if(mg_motor.params.angle <= -20000)  // -200度
            {
                mg_motor.params.angle = -20000;   // 限制在最低点
